...
def calculate_link_budget(orbital_params, ground_station, satellite_params, modem, modulation_bits_per_symbol):
    ...
    B = modem["bandwidth"]         # Bandwidth in Hz
    modulation = modem["modulation"]
    bits_per_symbol = modulation_bits_per_symbol.get(modulation, 1)
    
    # Calculate FSPL
    fspl = calculate_fspl(orbital_params["altitude"], orbital_params["frequency"])
    ...
    P_r_uplink_dBm = EIRP_ground_dBm - fspl + G_satellite_rx_dBi - L_uplink_dB
    ...
    P_r_downlink_dBm = EIRP_satellite_dBm - fspl + G_ground_rx_dBi - L_downlink_dB
    ...
    # Determine maximum bits per symbol achievable given SNR
...
# Function to select appropriate ground station
def select_ground_station(orbital_params, ground_segment):
    for station in ground_segment:
        if station["sup_freq"][0] <= orbital_params["frequency"] <= station["sup_freq"][1]:
            return station
    return None

# Function to select appropriate modem/SDR
...
if ground_station is None:
    print("No suitable ground station found for the given frequency.")
elif modem is None:
    ...
    # Visualization of radiation patterns (unchanged)
    theta = np.linspace(0, 2 * np.pi, 360)
    antenna_pattern = ground_station["sdown_gt"] * np.cos(theta)**2
    satellite_pattern = uplink_power_dBm - np.abs(theta - np.pi / 2) * 1.5  # Simplified
    
    plt.figure(figsize=(10, 5))
    plt.polar(theta, antenna_pattern, label="Ground Station Downlink Pattern")
    plt.polar(theta, satellite_pattern, label="Satellite Uplink Pattern")
